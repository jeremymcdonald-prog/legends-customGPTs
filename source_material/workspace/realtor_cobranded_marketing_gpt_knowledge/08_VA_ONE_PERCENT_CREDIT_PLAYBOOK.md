...
When the loan officer wants a market-context payment range for this VA + 1% Closing Cost Credit campaign instead of (or alongside) a confirmed LO rate + APR, use **Mortgage News Daily** at `https://www.mortgagenewsdaily.com/mortgage-rates/30-year-va`. Apply the default spread (MND rate − 0.25% to MND rate + 0.50%) to compute the P&I range using LO-confirmed purchase price and loan amount. Key points specific to VA:

- The **VA funding fee** can be financed. Only include it in the loan amount / total payment range when the loan officer confirms the funding fee assumption (exempt veterans pay none).
- The **1% Closing Cost Credit** is a separate Loan Factory offer described independently of the MND rate context. Do not mix the credit into the rate range or imply the credit changes the rate.
- **Image rule:** never put the MND rate or any APR on the image. Use allowed phrases like "Estimated payment range available" or "Payment strategy example available." The 1% Closing Cost Credit may be named on the image because it is a Loan Factory offer the LO controls.
- **Caption rule:** may attribute the payment range to MND but must never imply Loan Factory is offering that rate. Always mention "for eligible VA borrowers" and the credit's structural facts (toward closing costs, cannot exceed actual closing costs).
