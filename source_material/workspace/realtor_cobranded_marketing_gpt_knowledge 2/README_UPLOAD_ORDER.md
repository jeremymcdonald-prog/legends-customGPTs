...
### Step 7 — Upload Knowledge Files (in this order)
...
**Do NOT upload these files to Knowledge** (they live in the folder for development reference and for the Actions panel):

- `GPT_ACTION_RATE_QUOTE_OPENAPI_SCHEMA.json` — paste into the Custom GPT **Actions** panel, not Knowledge.
- `RATE_QUOTE_RESEARCH.md` — internal research log for the development team.
- `N8N_RATE_QUOTE_WORKFLOW_SPEC.md` — internal n8n workflow build spec.
...
**Optional / Phase 2:** A `getRateQuote` Action is designed and ready to wire up via an n8n webhook (see `N8N_RATE_QUOTE_WORKFLOW_SPEC.md`). It returns market-context rate data from Loan Factory's public chart — not a quote, not an approval. The GPT works fine without it.

This is **just** the production Custom GPT knowledge pack. You can build the rate quote automation on top of it as a Phase 2 once n8n + Playwright worker are deployed.
...
├── RATE_QUOTE_COMPLIANCE_RULES.md
├── RATE_QUOTE_RESEARCH.md
├── N8N_RATE_QUOTE_WORKFLOW_SPEC.md
├── GPT_ACTION_RATE_QUOTE_OPENAPI_SCHEMA.json
